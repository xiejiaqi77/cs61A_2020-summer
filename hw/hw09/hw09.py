...
def in_order_traversal(t): ## Yeah~~ one time pass
    """
    ...
    def helper(t, out):
        if t.is_leaf():
            out.append(t.label)
        if not t.is_leaf():
            helper(t.branches[0], out)
            out.append(t.label)
            helper(t.branches[1], out)
    helper(t, out)
    return out
...
def interleaved_sum(n, odd_term, even_term):
    ...
    total = 0
    i = 1
    def helper(i, the_last_term):
        nonlocal total
        if i == n:
            if the_last_term == "odd":
                total += even_term(i)
                return total
            elif the_last_term == "even":
                total += odd_term(i)
                return total
        else:
            if the_last_term == "odd":
                total += even_term(i)
                the_last_term = "even"
                helper(i + 1, the_last_term)
            elif the_last_term == "even":
                total += odd_term(i)
                the_last_term = "odd"
                helper(i + 1, the_last_term)
    helper(i, "even")  # if the helper did not contain the parament "total", then the "total" should be mark nonlocal in the helper function
    return total
...
def mutate_reverse(link):
    ...
    ###Recursion#### still have return value and did not mutate the original link
    """
        if link.rest is Link.empty:
        return link
    last = mutate_reverse(link.rest)
    link.rest.rest = link
    ...
    ####Iterative ##### still have return value and did not mutate the original link
    """
        if link.rest is Link.empty:
        return link
    curr = Link.empty

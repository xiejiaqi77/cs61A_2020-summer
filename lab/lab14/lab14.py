...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            s, cost = helper_align(skeleton_idx + 1, code_idx + 1)
            new_s = code_char + s
            possibilities.append((new_s, cost))
        # Insert
        s, cost = helper_align(skeleton_idx, code_idx + 1)
        new_s = "+[" + code_char + "]" + s
        possibilities.append((new_s, cost + 1))
        # Delete
        s, cost = helper_align(skeleton_idx + 1, code_idx)
        new_s = "-[" + skel_char + "]" + s
        possibilities.append((new_s, cost + 1))
...
def insert(link, value, index):
    ...
    """
    if index == 0:
        # head = Link(value, link)  => did not mutate the link itself directly? it mutate the link but did not change the 'self' 
        #link = head                => did not mutate the link itself directly?

...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        #return procedure.apply(args,env) also work # if wrote like this, it will be failed in Problem 11
        #return procedure.apply(args, env)   # if wrote like this, it will be failed in Problem 11
        return scheme_apply(procedure, args, env)   #also work
...
def eval_all(expressions, env):
    ...
    while expressions:
        ...
        # scheme_eval(val, env) can not be here, must be in the "else", or the final express value may be doubled
        # ex: scm> (begin (define x (+ x 1))
        #....        (define x (+ x 10))
        #....        (define x (+ x 100))
        #....        (define x (+ x 1000)))
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            ...
            return self.parent.lookup(symbol)  # point: find the parent
        # END PROBLEM 2
    ...
    def make_child_frame(self, formals, vals):
        ...
        #    raise SchemeError('the number of vals do not match the number of symbols')

        while formals and vals:
             frame_c.define(formals.first, vals.first)
             formals = formals.rest
             vals = vals.rest
             if (not vals and formals) or (vals and not formals):
                 raise SchemeError('the number of vals do not match the number of symbols')
        return frame_c
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        return self.env.make_child_frame(self.formals, args)  #Hint: make sure you're using self.env not env
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        results = scheme_eval(expressions.rest.first, env)   # must be expressions.rest.first
        env.define(target, results)
        return target
        # END PROBLEM 5
        ...
        "*** YOUR CODE HERE ***"
        f_name = target.first
        f = do_lambda_form(Pair(target.rest, expressions.rest), env)  #Point 搞清楚结构啊
        env.define(f_name, f)
        return f_name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    if expressions is nil:   # if expressions == None not work
        return True
    while expressions:
        val = expressions.first
        if not expressions.rest:
            last_value = scheme_eval(val, env)
...
def do_cond_form(expressions, env):  #need to be reviewed
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test): 
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            ############ My solution ########### which will fail at at final scheme.test
            #####fail case####
            # if there is no result for the cond
            # 1) the test is a number or something else can be returned, then return the
            # 2) the test is '#t' then return the test
            ...
            if result is None:     
                return test     # return True dose not work
            return result
            

            """
            #if above is 'return True';
            ...
            ########## other's solution ######## which will not fail at final scheme.test  :(   
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    #bindings = ((x 5) (y 4))
    #val = bindings.first => (x 5)
    #val.first = x
    #val.rest = (5 nil)
    while bindings is not nil:
        val = bindings.first  # val = (x 5)
        validate_form(val, 2, 2) 
        names = Pair(val.first, names)  # val.first = x
        values = Pair(scheme_eval(val.rest.first,env),values)  # val.rest = (5 nil) val.rest.first = 5
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 15   # 一次过？？其实也没有怎么懂
    "*** YOUR CODE HERE ***"
    def make_call_frame(self, args, env):
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a lexically-scoped call evaluated in my parent environment."""
        return env.make_child_frame(self.formals, args)
    # Hit：there must be env.make_child_frame instead of self.env.make_child_frame
    #  so the 'env' will be looked back up to the 'do_mu_form' where is the call expression was executed
    # END PROBLEM 15
...
## TURN TO BE RecursionError: maximum recursion depth exceeded in comparison

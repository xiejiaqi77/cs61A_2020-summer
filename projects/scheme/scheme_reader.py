...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        return Pair(quotes[val], Pair(scheme_read(src), nil))  #good
        # END PROBLEM 6
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
            src.pop_first() #point!! or there will be a '(' remained
            return nil
            # END PROBLEM 1
        else:
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
            return Pair(scheme_read(src), read_tail(src))  #point, the first must be sechme_read(src)
            # END PROBLEM 1
    except EOFError:
        raise SyntaxError('unexpected end of file')
